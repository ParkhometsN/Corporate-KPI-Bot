...
- `TELEGRAM_PROXY_URL` — HTTP/HTTPS proxy для Telegram Bot API, если VPS не открывает `api.telegram.org`.
...
- Если сам VPS не открывает `https://api.telegram.org`, используйте другой VPS или задайте `TELEGRAM_PROXY_URL`.
